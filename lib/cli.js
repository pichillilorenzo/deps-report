      ...
      for (let entry in result) {
        ...
        if (!result[entry].files) 
          ...
          for (let dependency of result[entry].dependencies) 
            console.log( ((i < 9) ? "  " : " ") + (++i) + ") " + ((options.parent.absPath) ? dependency.importAbsolutePath : dependency.importPath) )
        console.log("")
      }
    }
...
commander
  ...
  .action((glob/*: string*/, otherGlobs/*: Array<string> */, options/*: {root: string, parent: {excludeNodeModules: boolean, json: boolean, pretty: boolean, absPath: boolean, webpackConfig: string} }*/) => {
      ...
      for (let entry in result) {
        ...
        if (!result[entry].files) 
          ...
          for (let dependent of result[entry].dependents) 
            console.log( ((i < 9) ? "  " : " ") + (++i) + ") " + ((options.parent.absPath) ? dependent.fileAbsolutePath : dependent.filePath) )
        console.log("")
      }
    }
