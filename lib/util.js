...
function isRequireStatement(imp/*: Object */, isTS/*: boolean */ = false) /* : boolean */ {
  return (isTS) 
    ? (imp.kind == typescript.SyntaxKind.VariableDeclaration && imp.initializer && imp.initializer.expression && imp.initializer.expression.escapedText == "require") 
    : (imp.type == 'VariableDeclarator' && imp.init && imp.init.type == "CallExpression" && imp.init.callee.name == "require") 
}

function isImportStatement(imp/*: Object */, isTS/*: boolean */ = false) /* : boolean */ {
  return (isTS) 
    ? (imp.kind == typescript.SyntaxKind.ImportDeclaration) 
    : (imp.type == 'ImportDeclaration') 
}
